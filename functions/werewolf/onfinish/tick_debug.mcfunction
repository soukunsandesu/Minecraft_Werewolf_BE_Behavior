...
scoreboard players set @e[scores={CurrentRole=1..},hasitem={item=diamond},type=armor_stand] a_live -1

execute if score debug NumOfFox matches 1.. if score debug NumOfWolf matches 0 as @p run function werewolf/onfinish/winner/fox
execute if score debug NumOfFox matches 1.. if score debug NumOfVillagers matches 0 as @p run function werewolf/onfinish/winner/fox
